...
# MAGIC %md
# MAGIC # Step 4: RAG Chain (Databricks Vector Search + Databricks LLM)
# MAGIC
# MAGIC This notebook:
# MAGIC 1. Connects to the Databricks Vector Search index
# MAGIC 2. Builds a RAG pipeline: question -> Vector Search -> Databricks LLM -> answer with citations
...
# MAGIC %pip install langchain langchain-community langchain-community databricks-vectorsearch mlflow sentence-transformers faiss-cpu
...
# Load API key directly (avoids the os.environ caching bug)
...
print("Databricks LLM initialized")
...
def search_and_answer(question: str, k: int = 10):
    ...
    3. Databricks LLM generates answer with citations
...
What we built:
  ...
  - Databricks LLM (Llama 3.1 70B) for answer generation with citations
